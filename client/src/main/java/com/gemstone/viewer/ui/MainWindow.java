...
public class MainWindow extends JFrame {
    ...
    private JPanel buildEtsyPanel() {
        ...
        JButton importBtn = new JButton("⬇ Import Etsy");
        importBtn.setBackground(new Color(226, 90, 36)); importBtn.setForeground(Color.WHITE);
        importBtn.setFocusPainted(false); importBtn.setBorderPainted(false); importBtn.setOpaque(true);
        importBtn.setFont(new Font("Segoe UI", Font.BOLD, 12));
        importBtn.setPreferredSize(new Dimension(130, 32));
        importBtn.addActionListener(e -> openEtsyImport());

        etsyCountLabel = new JLabel("—");
        etsyCountLabel.setFont(new Font("Segoe UI", Font.BOLD, 12));
        etsyCountLabel.setForeground(ETSY_COL);
        actionRow.add(etsyCountLabel); actionRow.add(importBtn); actionRow.add(newBtn);
    ...
    private void openEtsyImport() {
        new EtsyImportDialog(this, db, () -> SwingUtilities.invokeLater(() -> {
            performEtsySearch();
            loadStats();
        })).setVisible(true);
    }
